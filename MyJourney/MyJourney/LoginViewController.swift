...
class LoginViewController: UIViewController, UITextFieldDelegate, DisplayMessageDelegate {
    ...
    @IBAction func login(_ sender: Any) {
        ...
        Task {
            let success = await firebaseController?.login(email: email, password: password)
            if success != "" {
                displayMessage(title: "Failed to login", message: success ?? "Failed to login", controller: self)
                return
            }
    ...
    @IBAction func displaySignup(_ sender: Any) {
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
    ...
    override func viewDidLoad() {
        ...
        if let login = databaseController?.getLogin() {
